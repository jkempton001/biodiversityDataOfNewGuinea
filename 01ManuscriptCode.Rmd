...
```{r}
frogDescriptionYrs = read_csv("ngAmphibiansInventory.csv")

# 1) Summarise descriptions by year (bars) + cumulative (line)
desc_yearly <- frogDescriptionYrs %>%
  mutate(desc_year = as.integer(year)) %>%      # assumes your column is literally named 'year'
  filter(!is.na(desc_year)) %>%
  distinct(binomial, desc_year) %>%              # ensures no duplicated species entries
  count(desc_year, name = "n_species") %>%
  arrange(desc_year) %>%
  mutate(cum_species = cumsum(n_species))

# 2) Scaling factor for secondary y-axis
scale_factor <- max(desc_yearly_species$cum_species) / max(desc_yearly_species$n_species)
...
# 3) Plot
frogDescYrsPlot = ggplot(desc_yearly, aes(x = desc_year)) +
  geom_col(
...
birdDescriptionYrs = read_csv("ngBirdsInventory.csv")

# 1) Summarise descriptions by year (bars) + cumulative (line)
desc_yearly <- birdDescriptionYrs %>%
  mutate(desc_year = as.integer(year)) %>%      # assumes your column is literally named 'year'
  filter(!is.na(desc_year)) %>%
  distinct(ScientificName, desc_year) %>%              # ensures no duplicated species entries
  count(desc_year, name = "n_species") %>%
  arrange(desc_year) %>%
  mutate(cum_species = cumsum(n_species))
...
# 3) Plot
birdDescYrsPlot = ggplot(desc_yearly, aes(x = desc_year)) +
  geom_col(
    aes(y = n_species * scale_factor),
    fill = "grey80",
    color = NA
...
desc_yearly <- mammalSEAsiaOccData %>%
  mutate(
...
# 3) Plot
mammalDescYrsPlot = ggplot(desc_yearly, aes(x = desc_year)) +
  geom_col(
    aes(y = n_species * scale_factor),
    fill = "grey80",
    color = NA
...
# 1) Extract description year and count unique species per year
desc_yearly <- plantSEAsiaOccData %>%
  mutate(
    desc_year = str_extract(authority, "(?<!\\d)(17\\d{2}|18\\d{2}|19\\d{2}|20\\d{2})(?!\\d)"),
    desc_year = as.integer(desc_year)
  ) %>%
...
# 3) Plot
plantDescYrsPlot = ggplot(desc_yearly, aes(x = desc_year)) +
  geom_col(
    aes(y = n_species * scale_factor),
    fill = "grey80",
    color = NA
