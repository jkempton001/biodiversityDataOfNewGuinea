...
frogSEAsiaOcc <- read_csv("gbifSEAsiaFrogsOccDataCleaned_20260103.csv")
...
## Fig. 1: Plot heatmap of occurrence data
...
frogCollPlot <-
  ...
  geom_sf(data = frogSEAsiaCollSF, col = "yellow", pch = 20, cex = 0.8) #+
...
frogPointMap
  ...
  ggsave(
    filename = paste0("frog", "_pointmap", ".svg"),
    plot     = frogPointMap,
    width    = 8,
    height   = 6,
    units    = "in",
...
# Import the combined raw dataset from GBIF download script
cat("Loading raw occurrence data...\n")
mammalSEAsiaOcc <- read_csv("gbifSEAsiaMammalOccDataCleaned_20260105.csv")
...
mammalCollPlot <-
  ggplot() +
  geom_sf(data = ng, fill = "black", linewidth = 0.2) +
  geom_sf(data = mammalSEAsiaCollSF, col = "yellow", pch = 20, cex = 0.8) #+
  #scale_x_continuous(breaks = c(-84)) +
  #theme(
    #plot.background = element_rect(fill = "#f1f2f3"),
    #panel.background = element_rect(fill = "#2F4051"),
...
mammalPointMap <-
  ...
  ggsave(
    filename = paste0("mammal", "_pointmap", ".svg"),
    plot     = mammalPointMap,
    width    = 8,
    height   = 6,
    units    = "in",
...
# Import the combined raw dataset from GBIF download script
cat("Loading raw occurrence data...\n")
birdSEAsiaOcc <- read_csv("gbifSEAsiaBirdOccDataCleaned_20260105.csv")
...
ggsave(
  filename = "birdSampleLocationsRasterMap.svg",  # file name and extension
...
birdPointMap <-
  ...
  ggsave(
    filename = paste0("bird", "_pointmap", ".svg"),
    plot     = birdPointMap,
    width    = 8,
    height   = 6,
    units    = "in",
...
# Import the combined raw dataset from GBIF download script
cat("Loading raw occurrence data...\n")
squamateSEAsiaOcc <- read_csv("gbifSEAsiaSquamateOccDataCleaned_20260105.csv")
...
ggsave(
  filename = "squamateSampleLocationsRasterMap.svg",  # file name and extension
...
squamatePointMap <-
  ...
  ggsave(
    filename = paste0("squamate", "_pointmap", ".svg"),
...
# Generate date stamp and save cleaned dataset ----
date_stamp <- format(Sys.Date(), "%Y%m%d")
output_filename <- paste0("gbifSEAsiaPlantOccDataCleaned_", date_stamp, ".csv")
...
# Import the combined raw dataset from GBIF download script
cat("Loading raw occurrence data...\n")
plantSEAsiaOcc <- read_csv("gbifSEAsiaPlantOccDataCleaned_20260105.csv")
...
ggsave(
  filename = "plantSampleLocationsRasterMap.svg",  # file name and extension
...
plantPointMap <-
  ...
  ggsave(
    filename = paste0("plant", "_pointmap", ".svg"),
    plot     = plantPointMap,
    width    = 8,
    height   = 6,
    units    = "in",
